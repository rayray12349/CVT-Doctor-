...
def detect_micro_slip(df, time_series):
    ...
    confidence = float(event_confirmed.sum()) / len(event_confirmed) * 100
...
def detect_short_time_slip(df, time_series):
    ...
    confidence = float(events.sum()) / len(events) * 100
...
def detect_forward_clutch_slip(df, time_series, tr690=True):
    ...
    downstream = df.get('Front Wheel Speed.1')  # Use RPM only
    if upstream is None or downstream is None:
        return False, None, 0.0

    delta = upstream - downstream
    mismatch = delta.abs().rolling(5).mean() > 75
    confidence = float(mismatch.sum()) / len(mismatch) * 100
...
def detect_torque_converter_judder(df, time_series):
    ...
    events = fluct & (throttle > 10) & (speed > 10)
    confidence = float(events.sum()) / len(events) * 100
    return events.rolling(10).sum().max() > 5, get_peak_time(events, time_series), confidence
...
def detect_chain_slip(df, time_series):
    ...
    events = overlap & (throttle > 10.0) & (gear > 1.5) & (speed > 10) & rpm_active
    confidence = float(events.sum()) / len(events) * 100
    return events.rolling(10).sum().max() > 5, get_peak_time(events, time_series), confidence
# ---------- Streamlit App ----------
...
if uploaded_file:
    ...
    results = {
        ...
        "Chain Slip": detect_chain_slip(df, time_series)
    }
    ...
    for label, (detected, peak_time, confidence) in results.items():
        if detected:
            ...
            st.markdown(f"- **{label}**: ⚠️ Detected{peak_str} — _{repair_actions[label]}_")
            st.markdown(f"&nbsp;&nbsp;&nbsp;&nbsp;• Confidence: **{confidence:.1f}%**")

            # Debug Chart
            st.markdown(f"#### 🔍 {label} Debug Chart")
            fig, ax = plt.subplots()
            if label in ["Micro Slip", "Short-Time Slip", "Long-Time Slip", "Chain Slip"]:
